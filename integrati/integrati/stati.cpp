...
void stati::makePolt()
{
    ...
    regen->setName("sponsor par type ");
    ...
    ticks << 1 << 2 << 3 << 4;
    labels << "Publicité" << "Matériel " << "Finance "  << "Marketing "  ;
    ...
    ui->customPlot->yAxis->setPadding(5); // a bit more space to the left border
     ui->customPlot->yAxis->setLabel("Nombre de sponsor par type \n ");
...
QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM sponsor where type LIKE '%publicite%' ");
    query.exec();
   ...
   // if (!query.isActive())
    ...
    QSqlQuery query1;
        query1.prepare("SELECT COUNT(*) FROM sponsor where type LIKE'%materiel%'");
        query1.exec();
        ...
        QSqlQuery query2;
            query2.prepare("SELECT COUNT(*) FROM sponsor where type  LIKE '%finance%' ");
            query2.exec();
           ...
           // if (!query2.isActive())
            ...
            QSqlQuery query3;
                query3.prepare("SELECT COUNT(*)FROM sponsor where type  LIKE '%marketing%'");
                query3.exec();
