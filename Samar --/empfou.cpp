...
FOURNISSEURS s;
produit p;
FOURNISSEURS e(IDE,NOM,PRENOM,EMAIL,TEL,p.recupererID(ui->produit->currentText()));
...
ajout->play();
    ...
    QString msg_vide="";
        ...
        FOURNISSEURS F(IDF,NOM,PRENOM,EMAIL,TEL,p.recupererID(ui->produit->currentText()));
        if ((!NOM.contains(regex1))||(!NOM.contains(regex2)))
        {
            ui->nom->setStyleSheet("QLineEdit { color: red;}");
...
void empfou::on_push_photo_clicked()
{
    son->play();
        ...
        if (QString::compare(filePh,QString())!=0)
        {
          ...
          if (valid)
          {
              ui->image->setPixmap(QPixmap::fromImage(PHOTO.scaled(281,231,Qt::KeepAspectRatio)));
