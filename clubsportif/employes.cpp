...
void EMPLOYES::afficher(Ui::integration *ui)
{
    QSqlQuery q;
    ...
    q.exec();
    modal->setQuery(q);
    ui->tableView_emp->setModel(modal);
}



bool EMPLOYES::Supprimer(Ui::integration *ui)
{
    QSqlQuery q;
    QString IDE = ui->tableView_emp->model()->data(ui->tableView_emp->model()->index(ui->tableView_emp->selectionModel()->currentIndex().row(),0)).toString();
    q.prepare("DELETE FROM EMPLOYES WHERE IDE ='"+IDE+"'");
    q.exec();
     return q.exec();
}

bool EMPLOYES::modifier(Ui::integration *ui)
{
...
/*
void EMPLOYES::Recherche(Ui::integration *ui)
{
    QSqlQuery q;
    QSqlQueryModel *modal=new QSqlQueryModel();
...
*/
void EMPLOYES::Tri_NOM(Ui::integration *ui)
{
    QSqlQuery q;
...
void EMPLOYES::Tri_PRENOM(Ui::integration *ui)
{
    QSqlQuery q;
    QSqlQueryModel *model=new QSqlQueryModel();
...
void EMPLOYES::Tri_SALAIRE(Ui::integration *ui)
{
    QSqlQuery q;
    QSqlQueryModel *model=new QSqlQueryModel();
