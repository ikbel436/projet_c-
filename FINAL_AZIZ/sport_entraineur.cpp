...
void sport_entraineur::on_tableView_doubleClicked(const QModelIndex &index)
{
    ...
    ui->nb_modif->setText(ui->tableView->model()->data(ui->tableView->model()->index(ui->tableView->selectionModel()->currentIndex().row(),3)).toString());
    ...
    ui->date_modif->setText(ui->tableView->model()->data(ui->tableView->model()->index(ui->tableView->selectionModel()->currentIndex().row(),2)).toString());
...
entraineur e(id,nom,prenom,dat,email,tel);
    if(e.modifier(ui))
    {
